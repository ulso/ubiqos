...
{
    if (lib) return true;
    lib = ubiqos_lib_link("sdlib", 0);
    if (!lib || lib->count <= SD_FORGET) { lib = 0; return false; }

    bool (*init)(const ubiqos_kernel_api_t *) =
        (bool (*)(const ubiqos_kernel_api_t *))lib->fn[SD_INIT_LIB];
    if (!init(&ubiqos_kernel_api)) { lib = 0; return false; }

    ubiqos_print("sd: library linked, running from the module pool\n");
    return true;
}
...
bool ubiqos_sd_have_driver(void)
{
