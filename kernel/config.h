...
bool myrtos_config_done(void);
void myrtos_config_give_up(void);   // no filesystem server: nothing is coming

// Never null. "myrtos" when the file said nothing, which is what the machine
// has always been called.
const char *myrtos_config_hostname(void);

// Empty when the file did not say. Empty is the question `wifi connect` asks;
// a value is the answer it does not have to.
const char *myrtos_config_ssid(void);
bool myrtos_config_has_password(void);

// "ssid\0password\0", the shape myrtos_wifi_join wants, or null when the file
