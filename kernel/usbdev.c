...
#define UBIQOS_USB_QUIET_US (15ull * 1000000ull)
...
void ubiqos_usb_init(void) {
    ...
    // ubiqos_ticks stays at 0 because pre-emption is enabled on the last line
    ...
    ubiqos_print("USB device started; the bus is joined once the task can answer it\n");
}
...
// set up once at boot and never taken down again. So UbiqOS could not notice,
...
uint32_t ubiqos_usb_suspends, ubiqos_usb_resumes;
uint32_t ubiqos_usb_mounts, ubiqos_usb_unmounts;
uint32_t ubiqos_usb_last_event_ms;
...
static void say_when(uint32_t count) {
    ubiqos_print(" at ");
    ubiqos_print_u32((uint32_t)(time_us_64() / 1000000u));
    ubiqos_print("s");

    char hhmmss[12];
    ubiqos_clock_time_only(hhmmss, sizeof hhmmss);
    if (hhmmss[0]) {
        ubiqos_print(" (");
        ubiqos_print(hhmmss);
        ubiqos_print(")");
    ...
    ubiqos_print(" (");
    ubiqos_print_u32(count);
    ubiqos_print(")\n");
}

static void note_event(const char *what, uint32_t count) {
    ubiqos_usb_last_event_ms = (uint32_t)(time_us_64() / 1000u);
    if (count > USB_EVENT_LOG_LIMIT) return;

    ubiqos_print("usb: host ");
    ubiqos_print(what);
    say_when(count);
}
...
// than that means UBIQOS_USB_QUIET_US is too short for this host rather than
...
uint32_t ubiqos_usb_longest_quiet_ms;   // the longest suspend seen
uint32_t ubiqos_usb_longest_at_s;       // uptime when that one began
static uint64_t quiet_began_us;

void tud_suspend_cb(bool remote_wakeup_en) {
    (void)remote_wakeup_en;
    quiet_began_us = time_us_64();
    note_event("suspended the bus", ++ubiqos_usb_suspends);
}
...
void tud_resume_cb(void) {
    if (quiet_began_us) {
        ...
        if (ms > ubiqos_usb_longest_quiet_ms) {
            ubiqos_usb_longest_quiet_ms = ms;
            ubiqos_usb_longest_at_s = (uint32_t)(quiet_began_us / 1000000u);

            // Always said, however many events have gone before: a new longest
            // is rare and is the one line a morning-after reading wants.
            char hhmmss[12];
            ubiqos_clock_time_only(hhmmss, sizeof hhmmss);
            ubiqos_print("usb: longest quiet so far ");
            ubiqos_print_u32(ms / 1000u);
            ubiqos_print("s, which began at ");
            ubiqos_print_u32(ubiqos_usb_longest_at_s);
            ubiqos_print("s");
            if (hhmmss[0]) { ubiqos_print(" ("); ubiqos_print(hhmmss); ubiqos_print(")"); }
            ubiqos_print("\n");
        }
        quiet_began_us = 0;
    }
    note_event("resumed the bus", ++ubiqos_usb_resumes);
}

void tud_mount_cb(void) {
    note_event("configured us", ++ubiqos_usb_mounts);
}
...
int32_t ubiqos_usb_write(const uint8_t *buf, uint32_t len) {
...
static void usb_thread(void) {
    ...
    for (;;) {
        ubiqos_usb_task();          // the console, on the hardware controller
...
#if UBIQOS_LWIP
        {
            extern void ubiqos_lwip_start(void);
            extern void ubiqos_lwip_poll(void);
            extern bool ubiqos_eh_netif_start(void);
            extern bool ubiqos_eh_netif_up(void);
            extern void ubiqos_eh_netif_poll(void);
            extern bool ubiqos_lwip_set_link(bool);
            static bool up;
            // Not before the card has been read: the hostname is in
            // /sd/config.txt, mDNS announces it once, and a responder that has
            // already said "ubiqos" cannot unsay it. ubiqos_config_done goes
            ...
            // below.
            if (!up && ubiqos_config_done()) {
                extern void ubiqos_lwip_sock_init(void);
                ubiqos_lwip_start();
                ubiqos_lwip_sock_init();   // this thread is stack 1's server
                up = true;
            }
            if (up) {
                ...
                if (tud_ready()) {
                    quiet_since = 0;
                    if (ubiqos_lwip_set_link(true))
                        note_link("up", ++ubiqos_net_link_ups);
                ...
                } else if (time_us_64() - quiet_since > UBIQOS_USB_QUIET_US) {
                    if (ubiqos_lwip_set_link(false)) {
                        note_link("down", ++ubiqos_net_link_downs);
                ...
                if (!ubiqos_eh_netif_up()) ubiqos_eh_netif_start();
                else                       ubiqos_eh_netif_poll();
                ...
                extern void ubiqos_sntp_poll(void);
                ubiqos_sntp_poll();

                extern void ubiqos_lwip_serve(void);
                ubiqos_lwip_serve();
            }
        }
#endif
        ...
        { extern void ubiqos_usbhost_drain(void); ubiqos_usbhost_drain(); }
        ubiqos_sleep(1);
...
void ubiqos_usb_start_task(void) {
    extern int32_t ubiqos_kernel_thread(void (*entry)(void), uint32_t stack_bytes,
                                        uint32_t priority);
    ...
    if (ubiqos_kernel_thread(usb_thread, 6144, UBIQOS_PRIO_USB) < 0) {
        ubiqos_print("USB: could not start its service process\n");
    }
}
...
uint32_t ubiqos_usb_available(void) {
    return tud_cdc_connected() || tud_mounted() ? tud_cdc_available() : 0;
}

int32_t ubiqos_usb_read(uint8_t *buf, uint32_t len) {
...
// --- NO DEVICE SIDE ----------------------------------------------------------
//
// Built with UBIQOS_NATIVE_USB=host the chip's controller is the host, and all
...
void ubiqos_usb_init(void) {
    ubiqos_print("USB: the port is a host; no console, disk or network on it\n");
}
...
void ubiqos_usb_start_task(void) {
    ...
    if (ubiqos_kernel_thread(usb_thread, 4096, UBIQOS_PRIO_USB) < 0) {
        ubiqos_print("USB: could not start its service process\n");
    }
}

#endif  // UBIQOS_USB_NATIVE_HOST
