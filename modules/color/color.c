...
static void chart(int32_t c) {
    ...
    for (uint32_t i = 0; i < 16; i++) {
        ...
        ubiqos_line_colour(&l, i, UBIQOS_BLACK);
        ubiqos_line_str(&l, "  ");
        ubiqos_line_str(&l, names[i]);
        uint32_t n = 0;
        while (names[i][n]) n++;
        while (n++ < 15) ubiqos_line_str(&l, " ");   // so the blocks line up
        ubiqos_line_colour(&l, UBIQOS_BLACK, i);
        ubiqos_line_str(&l, "      ");
        ubiqos_line_plain(&l);
        ubiqos_line_str(&l, "\r\n");
...
void module_main(int argc, char **argv) {
    ...
    const int32_t c = UBIQOS_STDOUT;

    if (argc < 2) {
        chart(c);
        ubiqos_write_str(c, "usage: color <text> [<background>], or color reset\r\n");
        return;
    }
    ...
    uint32_t bg = (argc > 2) ? by_name(argv[2]) : UBIQOS_KEEP;

    if (fg == UBIQOS_KEEP || (argc > 2 && bg == UBIQOS_KEEP)) {
        ubiqos_write_str(c, "color: no such colour\r\n");
        return;
    }
    ubiqos_colour(c, fg, bg);
}
