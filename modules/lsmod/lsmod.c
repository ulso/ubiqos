...
void module_main(void) {
    ...
    for (uint32_t i = 0; ; i++) {
        ...
        ubiqos_line_chars(&line, m.name, namelen);
        pad(&line, namelen, 17);
        ubiqos_line_str(&line, ubiqos_type_name(m.type));
        ubiqos_line_str(&line, "   ");
        ubiqos_line_u32(&line, m.revision);
        pad(&line, digits(m.revision), 5);
        ubiqos_line_u32(&line, m.links);
        pad(&line, digits(m.links), 7);
        ubiqos_line_u32(&line, m.size);
        ubiqos_line_str(&line, "\n");
        ubiqos_line_flush(UBIQOS_STDOUT, &line);
    }
}
