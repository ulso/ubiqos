...
static void put_hex2(ubiqos_line_t *l, uint32_t v) { ubiqos_line_hex_byte(l, v); }
...
static bool xfer(int32_t fd, uint8_t addr, const uint8_t *w, uint8_t nw, uint8_t nr) {
    ...
    return ubiqos_write(fd, buf, (uint32_t)(4 + nw)) >= 0;
}
...
void module_main(int argc, char **argv) {
    ...
    int32_t fd = ubiqos_open("/dev/i2c");
    if (fd < 0) { ubiqos_write_str(UBIQOS_STDERR, "i2c: no /dev/i2c\n"); return; }
    ...
    if (argc == 1) {
        ...
        uint32_t found = 0;
        ubiqos_line_reset(&l);
        ubiqos_line_str(&l, "scanning 0x08-0x77:");
        ...
        if (!found) ubiqos_line_str(&l, " nothing answered");
        ubiqos_line_str(&l, "\n");
    ...
    if (!from_hex(argv[1], &addr) || addr < 0x08 || addr > 0x77) {
        ubiqos_write_str(UBIQOS_STDERR, "i2c: address must be hex, 08 to 77\n");
        ubiqos_close(fd);
        return;
    }
    ...
    if (argc == 2) {
        ubiqos_line_reset(&l);
        put_hex2(&l, addr);
        ubiqos_line_str(&l, xfer(fd, (uint8_t)addr, 0, 0, 1) ? " answers\n" : " no answer\n");
        ubiqos_line_flush(UBIQOS_STDOUT, &l);
        ubiqos_close(fd);
        return;
    }
    ...
    if (argc == 5 && argv[3][0] == '=' && !argv[3][1]) {
        ...
        if (!from_hex(argv[2], &reg) || reg > 0xff || !from_hex(argv[4], &val) || val > 0xff) {
            ubiqos_write_str(UBIQOS_STDERR, "i2c: register and value are hex 00-ff\n");
            ubiqos_close(fd);
            return;
        }
        uint8_t w[2] = { (uint8_t)reg, (uint8_t)val };
        if (!xfer(fd, (uint8_t)addr, w, 2, 0))
            ubiqos_write_str(UBIQOS_STDERR, "i2c: no answer\n");
        ubiqos_close(fd);
        return;
    }
    ...
    if ((!raw && (!from_hex(argv[2], &reg) || reg > 0xff))
        || (argc > 3 && (!from_hex(argv[3], &count) || !count || count > UBIQOS_I2C_MAX_READ))) {
        ubiqos_write_str(UBIQOS_STDERR, "i2c: register is hex 00-ff or -, count 1 to 64\n");
        ubiqos_close(fd);
        return;
    }
    ...
    int32_t n = ubiqos_read(fd, got, count);
    ubiqos_line_reset(&l);
    for (int32_t i = 0; i < n; i++) { put_hex2(&l, got[i]); ubiqos_line_str(&l, " "); }
    ubiqos_line_str(&l, "\n");
    ubiqos_line_flush(UBIQOS_STDOUT, &l);
    ubiqos_close(fd);
}
