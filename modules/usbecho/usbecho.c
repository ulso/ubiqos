...
void module_main(void) {
    int32_t u = ubiqos_open("/dev/usb");
    if (u < 0) { ubiqos_exit(); return; }

    ubiqos_line_t line;
    for (int i = 0; i < 20; i++) {
        ubiqos_line_reset(&line);
        ubiqos_line_str(&line, "[usbecho] hello over USB CDC, line ");
        ubiqos_line_u32(&line, (uint32_t)(i + 1));
        ubiqos_line_str(&line, "\r\n");
        ubiqos_line_flush(u, &line);
