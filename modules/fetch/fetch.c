...
//
// This exists to prove ubiqos_sock_connect, which is the first outbound
...
void module_main(int argc, char **argv) {
    ...
    if (st != UBIQOS_TCP_ESTABLISHED) {
        say(st == UBIQOS_TCP_SYN_SENT ? "fetch: gave up waiting\r\n"
                                      : "fetch: could not connect\r\n");
        ubiqos_sock_close(sock);
        return;
    }

    // Built here rather than in a ubiqos_line_t: that holds 96 bytes, and a
    ...
    uint32_t done = 0;
    while (done < want) {
        int32_t n = ubiqos_sock_send(sock, (const uint8_t *)req + done, want - done);
    ...
    uint32_t quiet = 0;
    for (;;) {
        int32_t n = ubiqos_sock_recv(sock, buf, sizeof buf - 1);
