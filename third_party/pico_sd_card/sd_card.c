...
//    pio_set_wrap(pio, SD_DAT_SM, sd_cmd_or_dat_wrap_target, sd_cmd_or_dat_wrap);
    ...
    // LOCAL CHANGE, not upstream. These masks were 32 bits wide, and this board
    // has the card on GP34 to GP39. A shift of 36 into a uint32_t is taken modulo
    // 32 on RISC-V, so 0xf << 36 became 0xf << 4: the pin directions were set for
    // GPIO 2 to 7 instead. Nothing drove the card, the card never answered, and
    // the driver waited for ever -- which took the whole boot with it, since this
    // runs before there is a console to say so. The SDK's own note says the
    // 32-bit form only works for pins below 32.
    uint64_t dat_pin_mask = allow_four_data_pins ? 0xfull : 0x1ull;
    uint64_t all_pin_mask = (dat_pin_mask << sd_dat_pin_base) |
                            (1ull << sd_cmd_pin) | (1ull << sd_clk_pin);
    pio_sm_set_pindirs_with_mask64(sd_pio, SD_CLK_SM, all_pin_mask, all_pin_mask);
